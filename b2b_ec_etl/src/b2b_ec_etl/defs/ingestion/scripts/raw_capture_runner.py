...
from b2b_ec_pipeline.ingestion.models import ManifestBundle
...
@timed_run
def run_raw_capture(run_id: str | None = None) -> ManifestBundle:
    ...
    postgres_skipped = sum(1 for m in result.postgres if m.record_count == 0)
    file_skipped = sum(1 for m in result.files.values() if m.record_count == 0)
    logger.info(
...
@timed_run
def run_raw_capture_and_process(run_id: str | None = None) -> dict[str, object]:
    run_ts = datetime.now(timezone.utc)
    ...
    process_bad_rows = sum(m.bad_record_count for m in process_result.postgres) + sum(
        m.bad_record_count for m in process_result.files.values()
    )
...
@timed_run
def run_process_only(run_id: str | None = None) -> dict[str, object]:
    run_ts = datetime.now(timezone.utc)
